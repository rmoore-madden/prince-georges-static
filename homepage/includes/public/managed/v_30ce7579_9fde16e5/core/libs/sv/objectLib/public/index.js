...
define(function(require, exports, module) {
	...
	var filterArray = function(arr, filter) {
		var result = deepMap({ key : arr }, {
			key : {
				filterArray : {
	...
	var prefixToObject = function(obj, prefix) {
		var newObj = {};
		var prefixRe = new RegExp("^" + prefix + "(.*)");
		for(var i in obj) {
			var match = i.match(prefixRe);
			if (match) {
	...
	var prefixKeys = function(obj, prefix) {
		var newObj = {};
		for(var i in obj) {
	...
	var _bangJsMaskedValues = [];
	for(var i = 0; i < _bangJsMaskArgs.length; i++) {
		_bangJsMaskedValues.push(undefined);
	}
	
	var _bangJsRegex = /^!js!(.*)/;
	var bangJs = function(obj, args) {
		...
		var fnArgs = [null].concat(_bangJsMaskArgs).concat(keys);
		...
		traverse(obj, function(val) {
			...
			var match = val.match(_bangJsRegex);
			...
			var myArgs = fnArgs.concat("return " + match[1]);
			var fn = new (Function.prototype.bind.apply(Function, myArgs));
